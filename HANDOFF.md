...
| commit | fix | measured |
|---|---|---|
| `(this)` | **`train-sim` crashed on its FIRST optimise step — the DDQN target cell-mask was applied to the wrong tensor.** The cell head emits ONE placement map PER CARD (`ceqn`/`ceq2` are `(B, n_cards, n_cells)` = `[B, 10, 64]`), but `train_sim.py::optimise` tried to `masked_fill` that whole tensor with the DEPLOYABLE-cell mask `cellmask_next` shaped `[B, n_cells]` = `[B, 64]` (dim-1 mismatch), and then `argmax(1)`/`max(1)` reduced over the CARD dim, not the cell dim. `train_rl.py` already does this correctly via `_card_map(ceq, idx)` — reduce to the selected card's `(B, n_cells)` map FIRST, then mask + argmax. It only fired on the first batch (`RuntimeError: size of tensor a (64) must match tensor b (10)` at 0 matches), so `train-sim` could never train at all. | `python run.py train-sim --matches 10 --envs 4` CRASHED instantly → now runs clean: `[train-sim] stopped after 10 match(es); saved -> policy_sim.pt (0W-10L-0D)`. The only suite failure (`test_the_clamped_frontmost_ROW_counts_as_forward`, live X-Bow-into-push geometry) is unrelated to this change. (Command must run from `icebow/` — there is no root `run.py`.) |
